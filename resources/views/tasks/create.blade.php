...
<h3 class="text-3xl font-medium text-gray-700">Tambah Tugas untuk Proyek: {{ $project->nama_project }}</h3>
        ...
        <form action="{{ route('tasks.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <textarea name="deskripsi_tugas" id="deskripsi_tugas" rows="4" 
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('deskripsi_tugas') border-red-500 @enderror">{{ old('deskripsi_tugas') }}</textarea>
            ...
            <div class="mb-4">
                ...
                <select name="status_tugas" id="status_tugas" 
                    ...
                    <option value="in progress" {{ old('status_tugas') == 'in progress' ? 'selected' : '' }}>In Progress</option>
                    <option value="completed" {{ old('status_tugas') == 'completed' ? 'selected' : '' }}>Completed</option>
                </select>
                @error('status_tugas')
                    <p class="text-red-500 text-xs italic mt-2">{{ $message }}</p>
                @enderror
            ...
            <div class="flex justify-end">
                <a href="{{ url()->previous() }}" class="bg-gray-500 text-white px-6 mr-2 py-2 rounded-full hover:bg-gray-600">Kembali</a>
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded-full hover:bg-blue-600">Tambah Tugas</button>
            </div>
        </form>
    </div>
</div>
